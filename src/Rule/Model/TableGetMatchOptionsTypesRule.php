...
class TableGetMatchOptionsTypesRule implements Rule
{
    ...
    {
        $notArgsNames = ['primaryKey', 'finder', 'cache', 'cacheKey'];
        $options = [];
        $lastArgPos = 4;
        $totalArgsMethod = 5;
        if (
            ...
            return $this->getOptionsFromArray($args[$lastArgPos]->value, $notArgsNames, $options);
        }
        foreach ($args as $arg) {
            if ($arg->name && !in_array($arg->name->name, $notArgsNames)) {
                $options[$arg->name->name] = $arg->value;
            }
            $options = $this->extractOptionsUnpackedArray($arg, $notArgsNames, $options);
        }
